...
def check_results(output, context):
    if any(value is None for value in output):
        print("Could not parse findoffset results for {0:s}.".format(context))
        print("This usually means the findoffset command failed before writing BIC_c/trend/MJD.")
        print("Check findoffset.txt for the original Hector error message.")
        sys.exit(1)
    return output
...
for comp in range(0,n_comp):
    ...
    output = check_results(extract_results(), "component {0:d}, offset round 0".format(comp))
...
while i<8:
    ...
    for comp in range(0,n_comp):
        ...
        os.system("findoffset > findoffset.txt")
        output = check_results(extract_results(), "component {0:d}, offset round {1:d}".format(comp, i))
        print("MJD={0:f}, trend={1:f}, BIC_c={2:f}".\
					format(output[2],output[0],output[3]))
        #--- Add BIC_c value (associated before new jump is found) to total
        bic_c_0 = bic_c_0 + output[3]
